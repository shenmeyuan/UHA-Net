...
class Cephalometric(data.Dataset):  #人头测量
    ...
    def __init__(self, prefix, phase, transform_params=dict(), sigma=10, num_landmark=19, size=[640, 800], use_background_channel=False):
        ...
        self.pth_label_junior = os.path.join(prefix, 'junior')
        self.pth_label_senior = os.path.join(prefix, 'senior')

        # file index
        files = [i[:-4] for i in sorted(os.listdir(self.pth_Image))]  #数据划分
        n = len(files)
        if phase == 'train':
            self.indexes = files[:800]    
        elif phase == 'validate':
            self.indexes = files[800:900]
        elif phase == 'test':
            self.indexes = files[900:n]
        else:
        ...
        with open(os.path.join(self.pth_label_junior, name+'.txt')) as f1:
            with open(os.path.join(self.pth_label_senior, name+'.txt')) as f2:
                ...
                for i in range(self.num_landmark):
                    ...
                    points.append(tuple(round(p*new) for p, new,
